...
**백자청화죽문접시(운현궁)** 의 A4 1:1 실측도입니다. 위가 평면, 아래가 좌 반입면·우 반단면이고 둘은 회전축으로 세로를 맞춰 쌓았습니다. 이 접시는 옆에서 보면 13 mm밖에 되지 않아 구연과 굽 두 원의 중심을 잇는 선으로는 축이 서지 않습니다 — **두 원의 공통 법선**으로 정치한 것이 제목란의 `정치` 행입니다.

<p align="center">
  <img src="docs/images/dish-plate-detail.png" width="820" alt="입면·단면 띠 확대 — 빗금 친 단면, 수평 내선, 굽">
</p>

확대해 보면 실측 도면의 규칙이 그대로 있습니다. 오른쪽 반단면은 빗금이 쳐지고, 왼쪽 반입면에는 굽이 몸체에서 나뉘는 자리와 안바닥이 벽과 만나는 자리가 **한 바퀴 도는 수평 내선**으로 들어갑니다(외면 30°·내면 15° 이상, 폭 1 mm에서 읽은 꺾임). 안쪽 꺾임선은 단면선에서 종이 1 mm 앞에 멈추고, 유물 밖으로는 나가지 않습니다.

**이 도판은 검산할 수 있습니다.** SVG와 그 provenance를 저장소에 함께 넣어 두었습니다.
...
provenance에는 도형마다의 실측 크기(152.4 × 152.4 mm 평면, 152.3 × 13.4 mm 입면), 배치 종류(`plan_over_elevation/v1`), 중심축을 그린 근거가 된 Align recipe(`rotation_axis_from_circle_records/v1`), 꺾임선마다 실선·간선·생략 중 무엇이었는지, 그리고 유물 밖으로 나가려다 그려진 가장자리에서 멈춘 내선이 몇 개이고 얼마나 물렸는지까지 적힙니다. 종이 위의 선과 sidecar의 숫자는 같은 계산에서 나오므로 서로 어긋날 수 없습니다.

> **원본 자료에 관하여.** 저장소에는 **도면만** 있습니다. 스캔 메쉬와 텍스처는 들어 있지 않고 앞으로도 넣지 않습니다. 위 도판의 청화 대나무문은 프로그램이 채색을 제 색 그대로 따 붙일 수 있지만, 그 그림은 스캔 텍스처의 픽셀이므로 이 저장소용 도판에서는 뺐습니다(제목란 `문양` 행이 그렇게 적혀 있습니다). 원본 FBX는 실측자가 제공받은 자료이고, 공개 범위는 자료 제공처가 정합니다.

## 화면

<p align="center">
  <img src="docs/images/window.png" width="900" alt="ArchMeshRubbing 기본 창 — 오른쪽에 4축 작업 흐름 패널">
</p>

기본 창입니다. 오른쪽 패널이 작업 순서 그대로 정위치 · 실측용 도면 · 탁본 · 제원측정의 네 칸이고, 각 칸은 앞 단계가 `READY + FRESH`가 되기 전에는 열리지 않습니다. 지금은 메쉬가 없어 다음 할 일만 적혀 있습니다. (가운데 3D 화면은 이 문서를 만든 headless 환경에서 OpenGL context를 열 수 없어 비어 있습니다.)
