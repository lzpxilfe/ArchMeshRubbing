...
확대해 보면 실측 도면의 규칙이 그대로 있습니다. 오른쪽 반단면은 빗금이 쳐지고, 왼쪽 반입면에는 굽이 몸체에서 나뉘는 자리와 안바닥이 벽과 만나는 자리가 **한 바퀴 도는 수평 내선**으로 들어갑니다(외면 30°·내면 15° 이상, 폭 1 mm에서 읽은 꺾임). 안쪽 꺾임선은 단면선에서 종이 1 mm 앞에 멈추고, 유물 밖으로는 나가지 않습니다. 그 아래 파란 도장과 붉은 번호가 굽 안에서 따온 것입니다.
...
| 전개 탁본 | 기와 전개 record의 펴진 좌표 위에 같은 요철을 그린 탁본 (정치한 토기의 외면 띠 포함; 요철이 메쉬가 아니라 법선 지도에 있는 스캔은 그 지도에서). 기본은 둥근 원판으로 눌러 붙인 종이(`contact_envelope/v2`)의 양각 쪽 - 보고서 도판의 탁본처럼 바탕이 먹고 파인 곳·균열·자국이 흰 종이로 남으며 가장자리가 진해지고, 음각 쪽은 흰 바탕에 음각·균열·자국이 검게 | `.amr-rubbing` 1:1 PNG (sidecar 1.5.0) |
...
| 기와 전개 | 전체/선택 면, X/Y/Z 장축, Top/Bottom 해석, 자동/고정 seam, 왜곡 QC, 와통 축 정치(기록면 전체로 원통 맞춤), 언더컷 제외(펼 수 있는 면만 남기기) | `.amr-unwrap` OBJ·1:1 SVG |
