...
def _freeze_json(value: Any, *, field_name: str, depth: int = 0) -> Any:
    ...
    if isinstance(value, (float, np.floating)):
        ...
        if not math.isfinite(number):
            raise ArtifactVectorExtractionError(f"{field_name} contains a non-finite number")
...
def _json_mapping_copy(value: Mapping[str, Any], *, field_name: str) -> Mapping[str, Any]:
    try:
        encoded = json.dumps(
            _thaw_json(value),
            ensure_ascii=False,
...
class CutlineGeometryResult:
    ...
    def __post_init__(self) -> None:
        ...
        if VectorRecordKind(self.payload.kind) is not VectorRecordKind.CUTLINE:
            raise ArtifactVectorExtractionError("cutline result payload kind must be cutline")
...
def _validated_mesh_arrays(
    ...
    if np.any(face_array < 0) or np.any(face_array >= vertex_array.shape[0]):
        raise ArtifactVectorExtractionError("faces contain an out-of-range vertex index")
    duplicate_index = (
    ...
    twice_area = np.cross(triangles[:, 1] - triangles[:, 0], triangles[:, 2] - triangles[:, 0])
    if np.any(duplicate_index) or np.any(np.einsum("ij,ij->i", twice_area, twice_area) <= 0.0):
...
def _segment_for_triangle(
    ...
    if on_indices.size == 1:
        ...
        crossing = triangle[first] + interpolation * (triangle[second] - triangle[first])
    ...
    *,
    ...
    for root, indices in members.items():
        ...
        for index in indices:
            max_snap = max(
                max_snap,
                math.hypot(ordered[index][0] - center[0], ordered[index][1] - center[1]),
            )
...
def _collinear(
    ...
    *,
    ...
    if baseline <= tolerance:
        return False
    distance = abs(dx * (previous[1] - current[1]) - (previous[0] - current[0]) * dy) / baseline
...
def _remove_collinear_points(
    ...
    *,
    ...
    while True:
        ...
        if not removable:
            break
        candidate = [point for index, point in enumerate(result) if index not in removable]
...
def _paths_from_edges(
    ...
    *,
    ...
    branching = sorted(node for node, neighbors in adjacency.items() if len(neighbors) > 2)
    if branching:
        raise ArtifactVectorExtractionError(
    ...
    while remaining_nodes:
        ...
        start = min(component, key=lambda node: centers[node]) if closed else endpoints[0]
        ordered_nodes = [start]
        ...
        while True:
            ...
            if len(ordered_nodes) > len(component):
                raise ArtifactVectorExtractionError("section graph traversal did not terminate")
        if len(ordered_nodes) != len(component):
            raise ArtifactVectorExtractionError("section graph traversal omitted an edge")
        ...
        if closed:
            ...
            if abs(area) <= tolerance * tolerance:
                raise ArtifactVectorExtractionError("closed section path has zero planar area")
            if area < 0.0:
    ...
    def path_key(item: tuple[bool, list[tuple[float, float]]]) -> tuple[Any, ...]:
        ...
        return (0 if closed else 1, minimum_x, minimum_y, maximum_x, maximum_y, flattened)
...
def extract_cutline_geometry(
    ...
    *,
    ...
    on_plane = np.abs(face_signed) <= classification
    ...
    point_tangent_count = int(
        np.count_nonzero((on_counts == 1) & np.logical_xor(negative, positive))
    )
    ...
    return CutlineGeometryResult(payload=payload, qc=qc)
