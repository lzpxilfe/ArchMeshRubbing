...
The joint it repairs is the one a scanner actually leaves.  A boss, a lug or
a handle is a volume, so its skin meets the wall along **two** edges - one
where the upper surface runs in, one where the under surface does - and the
band of wall between them is buried inside the join, not visible surface.
The repair is therefore: delete that band, and sew each of the piece's two
edges to the edge the deletion made.  Sewing the piece's two edges to *each
other* instead - the obvious move, and the one that closes every boundary -
seals the piece into a lens and leaves it standing loose.
...
is the faces that straddle that zero.  The wall's *other* face is the same
distance away in space, so the search is kept to faces reachable across
shared edges from where the rings actually land - the outer skin of a pot is
a long walk from its inner skin even where the two are two millimetres
apart.
"""
...
#: The one repair this module performs.  Named as a version so that a second
#: kind of join - a ring to a ring, for a rim the scan stopped either side of
#: - can be added without changing what an existing document means.
SEW_RING_PAIR_TO_BODY = "sew_ring_pair_to_body/v1"
REPAIR_KINDS: tuple[str, ...] = (SEW_RING_PAIR_TO_BODY,)
...
def apply_mesh_repair(
    ...
    for join in joins:
        if join.kind != SEW_RING_PAIR_TO_BODY:
