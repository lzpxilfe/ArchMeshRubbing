        ...
        try:
            ...
            if event.button() == Qt.MouseButton.LeftButton:
                ...
                if self.picking_mode == "measure" and (modifiers & Qt.KeyboardModifier.ShiftModifier):
                    ...
                    try:
                        floor_pick_r = int(getattr(self, "_floor_pick_search_radius_px", FLOOR_PICK_SEARCH_RADIUS_PX_DEFAULT))
                    except Exception:
                        floor_pick_r = int(FLOOR_PICK_SEARCH_RADIUS_PX_DEFAULT)
                    floor_pick_r = int(max(2, min(floor_pick_r, 18)))
                    point = self.pick_point_on_mesh(
                        event.pos().x(),
                        event.pos().y(),
                        allow_depth_search=False,
                        search_radius_px=floor_pick_r,
                    )
                    if point is None:
                        # Fallback: small local depth search around the cursor.
                        point = self.pick_point_on_mesh(
                            event.pos().x(),
                            event.pos().y(),
                    ...
                    if point is not None:
                        # CAD AREA ????? ???醫딅르 ??? ??釉먮폇?????щ뮡嶺???숆강筌?????ш끽維??
                        # (?棺??짆?쏆춾???釉먮폇????繹먮끏裕??????????????⑤벡異?癲ル슢??????????野껊갭????節뚮쳮雅????됰씭???嶺뚮쮳?듬뤅癲?
                        surface_pt = self._snap_floor_point_on_picked_face(point)
                        if surface_pt is None:
                            self.status_info = "바닥점은 메쉬 표면에서만 선택됩니다. 표면을 다시 클릭하세요."
                            self.update()
                            return
                        world_pt = np.asarray(surface_pt[:3], dtype=np.float64)
                        ...
                        self.status_info = "野껊맧???袁⑹삺 癰귣똻???筌?????븍궦???????雅뚯눘苑??"
                        self.update()
                    return
                    ...
                    if point is not None:
                        surface_pt = self._snap_floor_point_on_picked_face(point)
                        if surface_pt is None:
                            self.status_info = "바닥면 선택은 메쉬 표면에서만 가능합니다. 표면을 다시 클릭하세요."
                            self.update()
                            return
                        vertices = self.pick_face_at_point(surface_pt)
                        if vertices:
