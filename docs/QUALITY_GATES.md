...
- 명시적 canonical 장축과 최대 250,000-face source selection, 자동 경계 또는 `[-180°, 180°)`의 0.000001° 고정 seam을 고정하고 자동 fallback, 1 µm grid collapse, topology·duplicate/non-manifold/orientation, 전역 positive-area UV overlap, 3-edge·area·Jacobian singular-value distortion gate(면 최대 25%·p95 15%·mean 7.5%는 `fit_per_section`에서만; 회전축 위의 `canonical_axis_origin`은 세 값을 모두 보고만 한다 - 그 전개에서 남는 것은 사상의 오차가 아니라 벽 자체의 요철이고, 타날문 기와의 등면은 승문의 기울기만으로 셋 다 넘어선다) 실패를 READY record로 승격하지 않는 `surface.tile_unwrap.v1`
- 동일 recipe 재계산과 canonical binary round-trip, source vertex/face correspondence, exact µm bounds·component hash·전체 payload SHA-256, Top/Bottom·자동/고정 seam 구분과 Align stale 보존. current 1.3 recipe/1.5 export와 byte-preserved 1.1·1.2·1.3 recipe와 1.1·1.2·1.3·1.4 export를 모두 버전 그대로 재계산·offline 검증
...
- versioned tile unwrap receipt 1.1/current export 1.6 Draft 2020-12 schema, byte-preserved export 1.1·1.2·1.3·1.4·1.5와 axis·seam·fallback·privacy closed contract; 1.4 이전 sidecar는 면 최대 distortion 25%를 넘는 record를, 1.5 이전 sidecar는 mean 7.5%·p95 15%를 넘는 record를, 1.6 이전 sidecar는 단면 평균 호 20°에 못 미치는 record를 정책과 무관하게 담지 못한다
...
- `.amr-rubbing` sidecar 1.5.0: `recipe.kind`가 receipt 종류(여섯 뷰 / 전개)와 provenance record type을 강제하고, 전개 receipt는 1.2.0 이상, 종이 기저 농담은 1.3.0 이상, 법선 지도 기복(`texture_relief`)은 1.4.0 이상, 눌러 붙인 종이(`contact_envelope/v2`)는 1.5.0에만 허용되며(v2가 주변 평균 필터·표본 하한·네모 창을 적으면 거부, v1은 여전히 그것들을 적어야 함), 1.0.0·1.1.0·1.2.0·1.3.0·1.4.0 schema bytes는 그대로 보존
...
- ~~**격자가 이어 붙인 조각은 조각 게이트를 지난다.**~~ **고쳤다 — outline algorithm 1.4.0, 벡터 sidecar 1.8.0.** 외형 조각 게이트(1.2.0, 아래)는 **스냅하고 closing한 뒤의** 조각 수를 세었다. 부스러기가 유물에서 격자 한두 칸 안에 떨어져 있으면 closing이 둘을 이어 붙여 한 조각이 되고, 게이트는 아무 말 없이 지났다. 합성 항아리(격자 0.2 mm) 옆 0.05 mm에 0.6 mm 부스러기를 놓아 재면, 1.3.0은 그것을 그렸다 — 조각 1개로 통과하고, 넓이는 7499.44 → 7499.40 mm²로 거의 그대로인데 그린 외형선 둘레가 333.92 → 335.21 mm로 **1.29 mm 늘었다.** 넓이가 움직이지 않으니 아무도 눈치채지 못하는 모양의 결함이다. 1.4.0은 **closing 전의 조각 수도** 본다: closing 전에 둘 이상이었는데 closing 뒤에 하나가 되었고 스냅 전 합집합도 하나가 아니면, 한 조각으로 만든 것은 유물이 아니라 closing이므로 거부하고 그렇게 말한다. closing 전에 이미 한 조각이던 외곽선은 1.3.0과 경로가 같다. 그 수는 QC `grid_pre_closing_component_count`로 남고, 1.4.0의 것이므로 1.3.0으로 쓴 기록은 QC 바이트가 그대로다. `tests/test_scan_defects.py`.

- ~~**Shapely를 올리면 지난 외곽선이 전부 무효가 된다.**~~ **고쳤다.** 외곽선 record의 검증은 recipe를 **지금 설치된** backend로 다시 만들어 대조했다. GEOS/Shapely 핀을 한 칸만 올려도 지난 모든 record의 recipe가 달라져 무효가 됐다 — 파일은 그대로인데. 이제 record가 적어 둔 자기 backend로 다시 만들고, 그 backend가 검토된 목록(`REVIEWED_OUTLINE_BACKENDS`)에 있는지만 본다. 검토되지 않은 backend로 만든 record는 여전히 거부한다. 중간 한도에 걸려 판정할 수 없을 때의 상태 이름(`unavailable_intermediate_limit`)도 함께 생겼다.

- ~~**양각 음영 전개가 이음매에서 짧아진다.**~~ **고쳤다.** 한 바퀴 도는 전개에서 각도 0을 걸치는 면은 θ가 0과 2π로 갈라져 뒤집힌 삼각형이 되므로 **버렸다**. 그래서 전개 띠가 둘레보다 짧아졌다. 이제 그런 면의 작은 쪽 모서리에 2π를 더해 펴서 그린다(버리지 않는다). 반지름 45 mm 그릇에서 띠가 둘레 2πR = 282.52 mm를 온전히 덮는다 — 전에는 8.82 mm 모자랐다. QC에 이음매를 걸친 면 수(`seam_face_count`)와 실제 먹이 닿은 폭(`development_inked_width_um`)이 남는다.

- ~~**ARAP이 조용히 물러난다.**~~ **고쳤다.** 메쉬가 크면 ARAP을 건너뛰고 초기 전개만 내놓으면서 결과에는 아무 표시가 없었다 — 반복 횟수만 남아 마치 수렴한 것처럼 보였다. 이제 `degraded`와 `degraded_reason`이 결과 meta에 실리고, 건너뛴 경우 반복 횟수는 0으로 적는다.

- **외곽선 격자가 메쉬보다 굵으면 구멍 난 도면이 통과한다.** 삼각형은 하나씩 격자에 스냅되므로, 투영이 한 칸보다 작은 삼각형은 넓이를 잃고 통째로 사라진다. 메쉬가 격자보다 고우면 그런 삼각형이 무더기로 나오고, 1칸 closing이 메우지 못하는 구멍이 실루엣 안에 남는다. 합성 암키와(150 mm, 50°, taper 0.10)로 잰 값:
  ...
  크게 어긋나면 `hole_not_strictly_inside`로 거부되지만, 그 사이 구간은 **구멍이 난 채로 통과한다.** 실제 박물관 스캔에서도 같은 값이 나온다 — 평면 1.0 mm에서 삼각형의 79%가 사라지고 구멍 10개가 남은 도면이 통과했다([`docs/REAL_DATA_TRIAL.md`](REAL_DATA_TRIAL.md)). 사라진 삼각형의 비율에 게이트를 두면 잡히리라고 한 번 적었다가 **거두었다.** 처음 계산이 분모를 잘못 잡았고(사라진 삼각형은 `projected_non_degenerate_triangle_count` 안에 이미 들어 있다), 바로잡은 값으로는 갈라지지 않는다 — 실제 토기의 정면은 1.0 mm 격자에서 44%가 사라지고도 구멍이 하나도 없고, 합성 기와는 35%에서 구멍 21개다. 남은 구멍 자체는 QC의 `hole_count`에 있으나 손잡이 고리나 굽 안쪽처럼 진짜 구멍과 격자가 뚫은 구멍을 숫자만으로 가르지 못한다. **가르는 것은 숫자가 아니라 스냅 전 합집합이었고, 이제 outline algorithm 1.3.0이 거부한다** — 아래 항목. GUI 기본값은 0.01 mm라 기본 설정으로는 걸리지 않는다.

- ~~**격자가 뚫은 구멍이 유물의 구멍처럼 그려진다.**~~ **고쳤다 — outline algorithm 1.3.0.** 넓이 비교를 위해 이미 만드는 스냅 전 합집합에는 격자가 뚫은 구멍이 없다 — 표면이 이어져 있으므로. 구멍마다 스냅 전 합집합이 덮는 넓이 비율을 재면 유물의 구멍(양쪽 벽 관통공)은 0.4 - 7%, 격자가 뚫은 구멍은 잰 것 전부 100%다(박물관 토기 평면 1.0 mm의 열 개, 0.5 mm의 두 개, 합성 항아리 200×60의 0.5 mm 네 개). 1.3.0은 50%를 넘는 구멍이 하나라도 있으면 거부하고 몇 개 중 몇 개가 격자의 것인지와 가장 큰 것의 mm 크기를 말하며, 통과한 외곽선은 최대 비율을 `grid_hole_unsnapped_cover_max`에 남긴다. 스냅 전 합집합이 없으면 구멍 있는 외곽선은 거부한다. 구멍이 유물의 것인 외곽선은 1.2.0과 바이트가 같다. 벡터 sidecar 1.7.0이 처음으로 1.3.0을 이름한다. `tests/test_scan_defects.py`, [`docs/REAL_DATA_TRIAL.md`](REAL_DATA_TRIAL.md).
