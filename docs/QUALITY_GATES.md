...
- 실제 complete-workflow `.amr`/`.amr-survey`를 exact-project로 다시 여는 field-pilot contract, project document/survey aggregate hash에 묶인 Windows 64-bit/native OpenGL receipt·닫힌 10항목 human review·정량 scale 교차 판정, canonical self-hash/no-overwrite publication, 절대 경로·hostname·사용자명 비수집과 `authentication=none`/single-pilot scope. 테스트의 합성 review/driver fixture는 계약 검증일 뿐 실제 현장 pilot 증거가 아님
