...
- **도판**: `DrawingSheetOptions.break_records`. 꺾임마다 그 높이·반지름의 원을 도형 frame에 사영한 현(`axis_profile_chord`)을 내선(`outline_hole`)으로 `profile-break:{record}:000` id로 긋는다. 축이 놓인 평면의 도형에만 그리고 평면도에는 아무것도 그리지 않는다. 어느 면을 읽었는가가 어디에 그릴지를 정한다: 바깥 면의 꺾임은 입면(outline 도형, 미러 도형의 입면 반쪽 — 축에서 잘린다)에, 안쪽 면의 꺾임은 단면(미러 도형의 단면 반쪽 — 축에서 내벽까지, 가운데선이 꺾인 상자 안은 잘려 나간다 — 또는 홀로 그린 cutline 도형 — 내벽에서 내벽까지)에 긋는다. 입면만 있는 도형은 안쪽 꺾임을 보일 수 없고, 단면만 있는 도형은 바깥 꺾임이 필요 없다(잘린 면이 이미 그린다); 둘 다 sidecar `profile_breaks.not_drawn`에 이유(`interior_needs_section_half`, `exterior_needs_elevation`)와 함께 남는다. sidecar `profile_breaks.records`(record ID·recipe hash·payload sha256·`break_count`·`break_heights_um`·`surface`)와 `profile_breaks.drawn`(`half` = `elevation`·`section`·`figure`, `surface`). 옵션이 비면 sidecar에 키가 없다.
- 완료 게이트(3/6/6)와 `.amr-survey`에는 들어가지 않는다.
