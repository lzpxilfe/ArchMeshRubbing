...
현재 writer는 `schemas/vector_export-1.4.0.schema.json`을 사용한다. 1.4는 1.3의 사용자 preset 정의 key set에 기법 선 종류 다섯(`technique_coil_joint` 등)을 더한 것이다 - 사용자 preset은 어휘 전체를 담으므로 어휘가 자라면 닫힌 key set도 새 판이 된다. 1.3은 1.2에 사용자 선 굵기 preset의 정의(`presentation.style_preset.definition`)를 더한 것이고, 현재 판이 아닌 sidecar는 사용자 preset으로 그린 도면을 담을 수 없다. 1.2는 1.1에 outline algorithm 1.1.0(격자 closing)의 recipe 키 `grid_closing`과 QC 키 넷을 더한 것이고, 1.1.0 이하 sidecar는 closing으로 계산한 outline을 담을 수 없다. 1.0.0·1.1.0·1.2.0·1.3.0 schema bytes는 그대로다. 1.1은 Align recipe/QC, geometry QC와 mesh-admission receipt, root payload QC를 exact-key로 닫고, record QC도 payload kind와 algorithm에 맞춘다. production Cutline/Outline은 해당 extractor QC 전부를 요구하며 custom algorithm은 공통 payload QC만 허용한다(Outline은 payload에서 재계산한 `outline_topology`도 필수). `vector_export-1.0.0.schema.json`은 바이트 불변 legacy 검증용으로 보존하며 reader는 admission receipt가 없던 유효 1.0 package를 계속 offline 검증한다.
