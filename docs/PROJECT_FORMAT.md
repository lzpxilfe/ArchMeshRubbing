...
- **recipe**: `archmeshrubbing.convex_dihedral_crease` 1.0.0. 결정하는 수 셋은 `detection_policy.dihedral_min_millideg`(기본 25000), `min_length_um`(기본 2000), `scale_um`(기본 0 — 모서리 하나의 이면각; 0보다 크면 모서리 양쪽으로 그 거리의 표면 법선 사이 각을 재고 mm당 꺾임이 가장 큰 모서리를 마루로 남긴다, `crest` 정책)이고, 볼록 판정(`far_corner_below_neighbour_plane/v1`)·용접 반경(`weld_um` 1)·가시성(`both_faces_toward_viewer/v1`)·사슬 순서·여섯 뷰 목록·자원 한도(사슬 4,096, 점 250,000)는 고정 정책으로 recipe에 적힌다. `validate_crease_recipe`는 세 수로 recipe를 다시 만들어 같은 바이트를 요구한다.
