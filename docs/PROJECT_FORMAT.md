...
- **획 규칙** (`valley` = `contact_envelope_stroke_centerline/v1`): 골 규칙과 다른 읽기. 탁본의 종이가 읽는 대로 읽는다 — 시문선보다 넓은 종이(`window_um` 기본 2,500, 탁본의 기준 반경과 같은 뜻)를 높이 위에 얹으면 종이는 홈 바닥에 닿지 않고, 종이 아래 `depth_um`(기본 120, 탁본의 검정 기준 깊이) 이상 파인 픽셀이 한 리본이 되며, 획은 그 리본을 한 픽셀로 세선화(Zhang-Suen)한 중심선이다. 리본은 제 회색 바닥 위로 `close_um`(기본 500)만큼 닫고, 뼈대의 `spur_um`(기본 800)보다 짧은 곁가지는 쳐낸다. 종이가 덮인 면 밖으로 나가는 가장자리(창의 반)는 읽지 않는다. 곡률이 픽셀마다 재는 것을 종이의 창이 대신하므로 종이가 건너뛰는 알갱이는 아예 보이지 않고, 획은 바닥이 옅어도 끝에서 끝까지 하나다. 이 규칙의 `detection_policy`는 `close_um`·`depth_um`·`incision_sign`·`line_smoothing_um`·`link_um`·`min_length_um`·`orientation_um`·`pattern`·`spur_um`·`straightness_min_percent`·`valley`·`window_um`이고 곡률 열쇠는 없다 — 골 규칙의 recipe는 예전 바이트 그대로다. **획은 열린 선이다.** 스스로 닫히는 뼈대(덩어리·도장 자국의 윤곽)는 억지로 획으로 만들지 않고 버리며, 양 끝 거리가 길이의 `straightness_min_percent`(기본 50)에 못 미치는 헤매는 사슬도 버린다(QC `closed_or_wandering_chain_count`). `orientation_um`(기본 1,500; 0이면 끔)은 사람이 무리를 보는 방식의 한 조각이다 — 깊이의 구조 텐서를 이 폭으로 평활해 픽셀마다 획이 달리는 방향과 그 일치도(coherence)를 얻고, 일치도 0.5 이상인 자리에서 그 방향과 40°를 넘게 어긋난 사슬(긁힘·깨진 가장자리·이음매)은 무리가 아니므로 버린다(`structure_tensor_coherence/v1`, QC `orientation_dropped_chain_count`).
- **패턴 묶기** (`pattern` = `strokes_by_direction_neighbourhood/v1`, 획 규칙에서만): 전개면 위에서 두 획의 가운데가 4 mm 안에 있고 방향(mod 180°)이 20° 안이면 이웃이고, 이웃 관계의 연결 성분 중 획 4개 이상이 패턴이다. 패턴은 벽의 위에서 아래로, 다음 왼쪽에서 오른쪽으로 번호를 받고, 어느 패턴에도 들지 않은 획은 **느슨한 획**(−1)이다. 각 패턴의 `direction_deg`(0-179)와 `line_count`를 적는다. 어골문의 좌우로 기운 띠는 띠마다 제 패턴이 된다 — 사람이 그렇게 보는 것과 같다.
