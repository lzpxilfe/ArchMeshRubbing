...
- **outline algorithm 1.2.0(현재)**은 1.1.0의 closing된 union이 **한 조각이 아니면 거부한다.** 연결된 메쉬의 실루엣은 연결되어 있으므로 외형 조각이 둘이면 유물일 수 없다 — 스캐너가 남긴 부스러기거나, 메쉬보다 굵은 격자가 유물을 끊은 것이다. 거부문은 조각 수와 가장 작은 조각의 넓이를 mm²로 말한다. 꼭짓점을 옮기지 않으므로 snap 계약은 1.1.0과 같고, 한 조각인 외곽선의 payload는 1.1.0과 바이트가 같다.
...
현재 writer는 `schemas/vector_export-1.6.0.schema.json`을 사용한다. 1.6은 1.5의 outline recipe `algorithm_version` enum에 `1.2.0`(조각 게이트)을 더한 것이고 그 이전 sidecar는 1.2.0 외곽선을 담지 못한다. 1.5는 1.4의 사용자 preset 정의 key set에 technique payload 1.2.0과 함께 들어온 정면 수법 넷(`technique_board_finishing`·`technique_burnishing`·`technique_interior_anvil`·`technique_paring`)을 더한 것이고, 1.4는 1.3에 기법 선 종류 다섯(`technique_coil_joint` 등)을 더한 것이다 - 사용자 preset은 어휘 전체를 담으므로 어휘가 자랄 때마다 닫힌 key set도 새 판이 된다. 1.3은 1.2에 사용자 선 굵기 preset의 정의(`presentation.style_preset.definition`)를 더한 것이고, 현재 판이 아닌 sidecar는 사용자 preset으로 그린 도면을 담을 수 없다. 1.2는 1.1에 outline algorithm 1.1.0(격자 closing)의 recipe 키 `grid_closing`과 QC 키 넷을 더한 것이고, 1.1.0 이하 sidecar는 closing으로 계산한 outline을 담을 수 없다. 1.0.0·1.1.0·1.2.0·1.3.0·1.4.0·1.5.0 schema bytes는 그대로다. 1.1은 Align recipe/QC, geometry QC와 mesh-admission receipt, root payload QC를 exact-key로 닫고, record QC도 payload kind와 algorithm에 맞춘다. production Cutline/Outline은 해당 extractor QC 전부를 요구하며 custom algorithm은 공통 payload QC만 허용한다(Outline은 payload에서 재계산한 `outline_topology`도 필수). `vector_export-1.0.0.schema.json`은 바이트 불변 legacy 검증용으로 보존하며 reader는 admission receipt가 없던 유효 1.0 package를 계속 offline 검증한다.
