...
- **outline algorithm 1.3.0(현재)**은 1.2.0에 더해 **격자가 뚫은 구멍을 거부한다.** 메쉬보다 굵은 격자는 모서리로 보이는 표면의 얇은 삼각형을 없애고, 그 자리에 closing이 메우지 못하는 구멍을 남긴다 — 박물관 토기 평면 1.0 mm에서 4 - 7 mm 구멍 열 개가 그렇게 그려졌다. 넓이 비교를 위해 이미 만드는 스냅 전 합집합에는 그 구멍이 없으므로, 구멍마다 스냅 전 합집합이 덮는 넓이 비율을 잰다: 유물의 구멍은 테두리의 snap 오차만큼만 덮이고(양쪽 벽 관통공 실측 0.4 - 7%), 격자가 뚫은 구멍은 전부 덮인다(실측 전부 100%). 어느 구멍이든 `grid_hole_gate.cover_fraction_max`(0.5)를 넘게 덮이면 거부하고 몇 개 중 몇 개가 격자의 것인지와 가장 큰 것의 크기를 mm로 말한다. 스냅 전 합집합이 없으면(GEOS 실패) 구멍 있는 외곽선은 거부한다. 통과하면 남은 구멍의 최대 비율을 QC `grid_hole_unsnapped_cover_max`(구멍이 없으면 0.0)에 남긴다. 꼭짓점을 옮기지 않으므로 snap 계약은 1.1.0과 같고, 구멍이 유물의 것인 외곽선의 payload는 1.2.0과 바이트가 같다. 1.2.0의 조각 거부문도 스냅 전 합집합으로 끊김(조각 1)과 부스러기(조각 2 이상)를 갈라 말한다.
- **outline algorithm 1.2.0**은 1.1.0의 closing된 union이 **한 조각이 아니면 거부한다.** 연결된 메쉬의 실루엣은 연결되어 있으므로 외형 조각이 둘이면 유물일 수 없다 — 스캐너가 남긴 부스러기거나, 메쉬보다 굵은 격자가 유물을 끊은 것이다. 거부문은 조각 수와 가장 작은 조각의 넓이를 mm²로 말한다. 꼭짓점을 옮기지 않으므로 snap 계약은 1.1.0과 같고, 한 조각인 외곽선의 payload는 1.1.0과 바이트가 같다.
- **outline algorithm 1.1.0**은 이 lattice union을 한 격자 반경의 morphological closing(`buffer(+1, mitre)` → `buffer(-1, mitre)` → `set_precision(1)`)으로 닫는다. 실루엣 근처의 투영 triangle은 격자보다 얇아서 개별 snapping이 그 사이에 한 격자 폭의 sliver hole과 외곽선이 한 점에서 맞닿는 pinch를 남기는데, 이것은 닫힌 표면에 없는, 격자의 인공물이다. closing은 두 격자보다 좁은 feature만 제거하며 `grid_closing_radius_cells`·`grid_closing_hole_fill_count`·`grid_closing_component_merge_count`·`grid_closing_area_delta_mm2`를 QC에, `grid_closing`을 recipe에 남긴다. snap 계약은 `axis<=1.5*grid; radial<=1.5*grid*sqrt(2)`(union 반 격자 + closing 반경 + 재snap 반 격자)다. **1.0.0 record**(closing 없음, `axis<=grid/2; radial<=grid/sqrt(2)`)는 recipe의 `algorithm_version`대로 예전 그대로 재계산되며, 상태 표기(`annotation.condition.v1`)의 뷰별 경계는 1.0.0 union을 그대로 쓰고, 기법 표기(`annotation.technique.v1`)는 recipe의 `algorithm_version`이 정한다 - 기법 1.0.0은 plain union, 1.1.0은 closing, 1.2.0은 closing에 더해 뷰 방향과 이루는 코사인이 0.15보다 작은 면(약 81° 넘게 기울어진 면)을 그 뷰에서 뺀다 - 실루엣까지 닿는 띠는 가장자리가 sliver로 투영되어 뷰 전체가 거부되므로. 그 뷰의 면이 전부 그렇게 빠지면 `grazing_view` 사유로 건너뛴다. 1.3.0(현재)은 거기에 더해 영역을 `extract_region_geometry`로 그린다 - 삼각형을 하나씩 격자에 맞춘 뒤 합치는 outline 방식 대신, 정확히 합친 뒤 한 번만 격자에 맞추고 closing한다. 조밀한 메쉬에 칠한 영역은 삼각형 개별 snapping이 closing으로도 못 메우는 sliver를 남겨 뷰가 거부되었기 때문이다. 경계는 여전히 격자 위에 있고 outline 1.1.0의 closing 계약 안에 있다. 거친 벽에 칠한 자국은 plain union에서 sliver로 갈라져 모든 뷰가 거부되므로 closing이 필요하다. 기법 payload는 1.1.0부터 `surface_side`(exterior/interior/mixed)·`interior_face_fraction_millionths`·`direction_deg`(null 가능)를 더 담고, 1.0.0 payload는 그 키 없이 예전 바이트 그대로 읽히고 검증된다. recipe의 `direction_deg`는 있을 때만 쓰이는 선택 키다.
...
현재 writer는 `schemas/vector_export-1.11.0.schema.json`을 사용한다. 1.11은 1.10의 outline recipe `algorithm_version` enum에 `1.5.0`(격자가 뚫은 구멍을 거부하는 대신 메쉬에 물어 닫는 판)을 더하고 QC 키 `grid_punched_hole_count`·`grid_punched_hole_max_mm2`를 닫힌 key set에 넣은 것이며, 그 이전 sidecar는 1.5.0 외곽선을 담지 못한다. 1.10은 1.9의 선 종류 어휘에 `section_presumed`(잰 면이 아닌 자리 위의 단면선)를 더한 것이고, 1.9는 1.8에 `section_mark`(단면 위치선)를 더한 것이다 — 사용자 preset은 어휘 전체를 담으므로 어휘가 자랄 때마다 닫힌 key set도 새 판이 된다. 1.8은 1.7의 outline recipe `algorithm_version` enum에 `1.4.0`(격자가 이어 붙인 조각까지 거부하는 게이트)을 더하고 QC 키 `grid_pre_closing_component_count`를 닫힌 key set에 넣은 것이며, 그 이전 sidecar는 1.4.0 외곽선을 담지 못한다. 1.7의 Align recipe·QC `axis_source` enum에는 `standing_on_foot/v1`(뒤틀린 그릇을 굽으로 바닥에 세운 자세; 실측자가 고른다)이 뒤에 더해졌고, 그 값을 담은 sidecar는 더해진 뒤의 1.7.0 schema로만 검증된다. 1.7은 1.6의 outline recipe `algorithm_version` enum에 `1.3.0`(구멍 게이트)을 더하고 recipe 키 `grid_hole_gate`와 QC 키 `grid_hole_unsnapped_cover_max`를 닫힌 key set에 넣은 것이며, 그 이전 sidecar는 1.3.0 외곽선을 담지 못한다. 1.6은 1.5의 outline recipe `algorithm_version` enum에 `1.2.0`(조각 게이트)을 더한 것이고 그 이전 sidecar는 1.2.0 외곽선을 담지 못한다. 1.5는 1.4의 사용자 preset 정의 key set에 technique payload 1.2.0과 함께 들어온 정면 수법 넷(`technique_board_finishing`·`technique_burnishing`·`technique_interior_anvil`·`technique_paring`)을 더한 것이고, 1.4는 1.3에 기법 선 종류 다섯(`technique_coil_joint` 등)을 더한 것이다 - 사용자 preset은 어휘 전체를 담으므로 어휘가 자랄 때마다 닫힌 key set도 새 판이 된다. 1.3은 1.2에 사용자 선 굵기 preset의 정의(`presentation.style_preset.definition`)를 더한 것이고, 현재 판이 아닌 sidecar는 사용자 preset으로 그린 도면을 담을 수 없다. 1.2는 1.1에 outline algorithm 1.1.0(격자 closing)의 recipe 키 `grid_closing`과 QC 키 넷을 더한 것이고, 1.1.0 이하 sidecar는 closing으로 계산한 outline을 담을 수 없다. 1.0.0·1.1.0·1.2.0·1.3.0·1.4.0·1.5.0·1.6.0·1.7.0 schema bytes는 그대로다. 1.1은 Align recipe/QC, geometry QC와 mesh-admission receipt, root payload QC를 exact-key로 닫고, record QC도 payload kind와 algorithm에 맞춘다. production Cutline/Outline은 해당 extractor QC 전부를 요구하며 custom algorithm은 공통 payload QC만 허용한다(Outline은 payload에서 재계산한 `outline_topology`도 필수). `vector_export-1.0.0.schema.json`은 바이트 불변 legacy 검증용으로 보존하며 reader는 admission receipt가 없던 유효 1.0 package를 계속 offline 검증한다.
...
현재 machine-readable 계약은 record receipt의 `schemas/tile_unwrap_receipt-1.1.0.schema.json`과 export sidecar의 `schemas/tile_unwrap_export-1.6.0.schema.json`이다. export schema는 자동 seam 1.1 recipe, 자동/고정 seam 1.2 recipe, 단면 중심·station 정책을 가진 1.3 recipe를 닫힌 형태로 구분하며, current writer는 1.6 sidecar를 만든다. 1.4·1.5·1.6이 1.3과 다른 것은 상한 넷뿐이고, 넷 다 `section_center_policy`가 `canonical_axis_origin`일 때만 풀린다: 1.4에서 `qc.record.distortion_max_millionths`가, 1.5에서 `distortion_mean_millionths`와 `distortion_p95_millionths`가 상한 없이 보고되고, 1.6에서 `section_mean_span_microdegrees`의 20° 하한이 없어진다. `fit_per_section`이면 넷 모두 1.3과 같은 25%·7.5%·15%·20°가 상한이다. 회전축 위에서 남는 것은 사상의 오차가 아니라 벽 자체의 요철이므로 - 승문을 새긴 기와 등면은 그 기울기만으로 셋을 모두 넘어선다 - 세 값은 거부의 근거가 아니라 읽는 이가 저울질할 기록이다. 20° 하한도 마찬가지로 원 맞춤의 것이다: 그보다 좁은 호에 놓인 점들은 중심을 잡아 주지 못하므로 맞춤이 못 미더운 것이지, 축 위에서 전개한 좁은 띠가 못 미더운 것이 아니다. 한 바퀴를 여러 장으로 나눠 뜨면 어느 장도 20°가 못 되고, 반지름 136 mm인 항아리에서 20°는 종이 48 mm다. 공개된 `tile_unwrap_export-1.1.0.schema.json`·`1.2.0`·`1.3.0`·`1.4.0`·`1.5.0`의 bytes와 recipe hash는 바꾸지 않고 runtime이 기존 패키지와 프로젝트를 계속 offline 검증·재계산한다. 1.4 이전 sidecar는 정책과 무관하게 면 최대 25%를, 1.5 이전 sidecar는 mean 7.5%·p95 15%를 넘는 record를, 1.6 이전 sidecar는 단면 평균 호가 20°에 못 미치는 record를 담지 못한다. 두 세대 모두 axis 추정값, fallback 허용값, 사설 경로와 계약 밖 필드를 거부하고 topology·전역 UV overlap·row-shift QC를 고정한다. `section_row_shift_station_count == section_count` 같은 cross-field 의미 제약은 runtime known-record validator가 최종 강제한다. 공개 릴리스 전에 사용된 실험적 1.0 schema 파일은 byte-exact 회귀와 계약 이력 검토를 위해서만 보존한다. 현재 runtime은 1.0 record/package와 이를 포함한 프로젝트를 읽지 않으며 자동 migration도 제공하지 않는다.
...
- **recipe**: `archmeshrubbing.convex_dihedral_crease` **1.1.0**(현재) 또는 1.0.0(고정). 결정하는 수는 `detection_policy.dihedral_min_millideg`(기본 25000), `min_length_um`(기본 2000), `scale_um`(기본 0 — 모서리 하나의 이면각; 0보다 크면 스케일에서 읽는다), 그리고 1.1.0에만 있는 `link_um`(기본 0 — 마주보는 사슬 끝을 잇는 거리)이다. 볼록 판정(`far_corner_below_neighbour_plane/v1`)·용접 반경(`weld_um` 1)·가시성(`both_faces_toward_viewer/v1`)·사슬 순서·여섯 뷰 목록·자원 한도(사슬 4,096, 점 250,000)는 고정 정책으로 recipe에 적힌다. 스케일 읽기의 마루 규칙은 `crest`에 적힌다: 1.0.0은 `turning_per_mm_maximum_within_half_scale/v1`(스캔에서 능선이 조각으로 끊겨 물러났다), 1.1.0은 `largest_curvature_maximum_zero_crossing/v2`이고 `sample`(`area_weighted_normals_within_half_scale/v1`)과 `link`(`ends_facing_within_45deg_4_rounds/v1`) 두 정책 키가 함께 들어간다([`docs/LITHIC_TRIAL.md`](LITHIC_TRIAL.md)). `validate_crease_recipe`는 record의 `algorithm_version`과 수들로 recipe를 다시 만들어 같은 바이트를 요구하며, 1.0.0 record는 1.0.0 규칙으로 다시 계산된다. 1.0.0 기본 recipe의 바이트는 `tests/test_crease_record.py`에 sha256으로 고정되어 있다.
...
- **recipe**: `archmeshrubbing.texture_normal_map_valleys` **1.0.0**. `view`(여섯 뷰 중 하나), `texture_relief`(탁본과 같은 블록 — 아틀라스·법선 지도의 sha256, `encoding`, `smoothing_um` 기본 2,000: 1 mm 폭 시문선은 1 mm 기저에 제 기울기의 대부분을 잃는다), `raster_policy`(`pixels_per_mm` 기본 5, `facing_cos_millionths` 기본 350,000 — 뷰 방향과 70° 넘게 기운 면은 읽지 않는다, `painter`), `detection_policy`(`scale_um` 기본 300 — 헤시안을 구하는 가우시안 폭, `curvature_min_per_m` 기본 300 — 골로 치는 최소 곡률 1/mm의 천분율, `curvature_seed_per_m` 기본 그 두 배 — 골 픽셀은 이어진 무리 어딘가가 이 곡률에 닿을 때만 남는다(히스테리시스: 낮은 문턱이 시문선을 희미한 끝까지 따라가고 높은 문턱이 알갱이를 걸러낸다), `link_um` 기본 1,000 — 이 거리 안에서 서로를 향한 사슬 끝을 잇는다(35° 안), `incision_sign` 기본 −1 — 지도의 법선이 벽 밖을 향하면 시문선은 높이의 골(−1)이고 안을 향하면 마루(+1)인데 파일은 어느 쪽인지 말하지 않으므로 QC가 두 부호의 픽셀 수를 모두 세어 준다(박물관 지도는 +1), `min_length_um` 기본 1,500, `line_smoothing_um` 기본 300 — 추적한 선의 평활 폭, `valley` = `hessian_largest_eigenvalue_zero_crossing/v1`). `validate_texture_lines_recipe`는 수들로 recipe를 다시 만들어 같은 바이트를 요구한다.
- **어디서 추적하는가** (`domain`): `view`는 정사영 뷰 위에서 그대로 읽는다 — 실루엣으로 갈수록 벽이 단축되어 1 mm 시문선이 픽셀 하나로 눌리고 곡률 문턱에 못 미친다. `axis_development`는 뷰를 향한 외벽 면(법선이 축에서 멀어지고 뷰 쪽 반절에 있는 면)을 정치한 축 기준으로 편다 — u는 그 꼭짓점 반지름에서 뷰 자오선까지의 호 r·Δθ, v는 축 높이 — 그 위에서 추적하고, 선의 점마다 놓인 전개 삼각형의 무게중심 좌표로 canonical 위치를 되찾아 뷰에 사영한다. 벽이 `facing_cos` 너머로 돌아간 자리에서 선을 끊으므로 실루엣 너머로 넘어가지 않는다. 손 실측이 하는 일과 같다: 문양은 탁본에서 베끼고 입면에 옮기며, 둘은 같은 읽기이므로 어긋나지 않는다. 축 Align이 있어야 하고 옆 뷰(front·back·left·right)에서만 된다. 언더컷으로 두 겹이 한 자리에 펴지면 축에서 먼 겹이 이긴다.
...
- **획 규칙** (`valley` = `contact_envelope_stroke_centerline/v1`): 골 규칙과 다른 읽기. 탁본의 종이가 읽는 대로 읽는다 — 시문선보다 넓은 종이(`window_um` 기본 2,500, 탁본의 기준 반경과 같은 뜻)를 높이 위에 얹으면 종이는 홈 바닥에 닿지 않고, 종이 아래 파인 깊이가 반 창 안의 가장 얕은 자리보다 `depth_um`(기본 80 — 탁본의 검정 기준 깊이 120보다 얕게 잡는다; 이웃에서 재므로 알갱이는 어차피 문턱을 넘지 못하고, 얕은 획의 줄이 살아난다) 이상 큰 픽셀이 한 리본이 되며 — 깊이를 고정된 선이 아니라 이웃에서 재는 것은, 빽빽한 문양에서는 획 사이의 마루도 종이 아래에 있고 얕은 획의 줄도 한 줄이기 때문이다 —, 획은 그 리본을 한 픽셀로 세선화(Zhang-Suen, 8-이웃 부호표)한 중심선이다. 종이는 높이의 **grey closing**(평평한 창)이다 — 평면과 완만한 배부름은 제 closing 그대로이고 창보다 좁은 오목한 곳만 메워진다. 국소 최댓값으로 종이를 잡으면 법선 지도를 적분한 높이의 기울기·표류 위에서 벽의 절반이 "종이 아래"가 되어 넓은 영역의 중심축이 획으로 나왔다. 리본은 제 회색 바닥 위로 `close_um`(기본 500)만큼 닫되 획들이 에워싼 칸은 채우지 않고(어골문의 꼭짓점이 다음 줄과 만나 이룬 칸을 채우면 한 줄이 한 덩어리가 되어 뼈대가 그물이 된다), 뼈대의 `spur_um`(기본 800)보다 짧은 곁가지는 쳐낸다. 종이가 덮인 면 밖으로 나가는 가장자리(창의 반)는 읽지 않는다. 곡률이 픽셀마다 재는 것을 종이의 창이 대신하므로 종이가 건너뛰는 알갱이는 아예 보이지 않고, 획은 바닥이 옅어도 끝에서 끝까지 하나다. 이 규칙의 `detection_policy`는 `close_um`·`depth_um`·`incision_sign`·`line_smoothing_um`·`link_um`·`min_length_um`·`orientation_um`·`pattern`·`pattern_gap_um`·`spur_um`·`straightness_min_percent`·`valley`·`window_um`이고 곡률 열쇠는 없다 — 골 규칙의 recipe는 예전 바이트 그대로다. **획은 열린 선이다.** 추적한 사슬은 먼저 0.6 mm 폭에서 60°를 넘게 꺾이는 자리마다 자른다 — 추적기에게 하나인 지그재그는 눈에는 팔마다 한 획이고 실측자도 그렇게 그린다. 그 뒤 스스로 닫히는 뼈대(덩어리·도장 자국의 윤곽)는 억지로 획으로 만들지 않고 버리며, 양 끝 거리가 길이의 `straightness_min_percent`(기본 70)에 못 미치는 헤매는 사슬도 버린다(QC `closed_or_wandering_chain_count`). `orientation_um`(기본 2,500; 0이면 끔)은 사람이 무리를 보는 방식의 한 조각이다 — 깊이의 구조 텐서를 이 폭으로 평활해 픽셀마다 획이 달리는 방향과 그 일치도(coherence)를 얻고, 일치도 0.5 이상인 자리에서 그 방향과 40°를 넘게 어긋난 사슬(긁힘·깨진 가장자리·이음매)은 무리가 아니므로 버린다(`structure_tensor_coherence/v1`, QC `orientation_dropped_chain_count`).
- **능선 규칙** (`valley` = `contact_envelope_stroke_ridge/v1`): 같은 종이를 획 하나하나로 읽는다. 1 mm 간격으로 빗질한 줄에서는 종이 아래 리본들이 서로 닿아, 문턱보다 깊은 픽셀의 중심선이 획 사이를 지나며 획을 돌아 고리가 된다 — 문턱은 두 획을 가르지 못하지만 깊이는 아직 가른다. 그래서 획을 **종이 아래 깊이가 줄을 가로질러 가장 큰 자리의 선**으로 잡는다: 픽셀마다 획이 달리는 방향을 종이 아래 깊이의 구조 텐서(`orientation_um` 폭; 0이면 1 mm)에서 얻고, 깊이(1 px 가우시안으로 고르고 그 방향을 따라 앞뒤 0.2 mm를 평균한 것)가 그 방향의 가로 두 이웃(1 px) 이상인 픽셀이 획이다. 선은 `depth_um`의 50%까지 따라가되 어딘가 `depth_um`에 닿는 무리만 남긴다(히스테리시스). closing은 없다 — 아무것도 메우지 않으니 아무것도 붙지 않는다. 그 뒤는 획 규칙과 같다(세선화·계단 줄이기·`spur_um` 곁가지 치기·꺾임 자르기·열린 선·방향장·패턴·문양대). `detection_policy`는 획 규칙에서 `close_um`을 뺀 것이고 `valley`가 이 이름이다. QC의 `stroke_pixel_count`는 두 규칙 모두 `depth_um` 이상 파인 픽셀 수다. 시험 토기에서 획 규칙은 5,502획을 읽으면서 빽빽한 동체의 절반을 사다리꼴 그물로 만들어 버렸고, 능선 규칙은 같은 자리를 획마다 읽는다([`docs/REAL_DATA_TRIAL.md`](REAL_DATA_TRIAL.md)).
- **채색 문양** (recipe의 `texture_relief` 대신 `texture_paint`): 자기는 탁본을 뜨지 않는다. 문양은 금채·청화·철화의 **채색**이고, 실측 도면은 그것을 선으로 그린다 — 그린 선은 그 가운데를 따라, 칠한 면은 그 가장자리로. 법선 지도가 시문한 토기에 대한 것이라면 base colour 지도는 채색한 자기에 대한 것이다: 같은 아틀라스, 같은 전개 격자(`rasterise_developed_texels`, 릴리프의 래스터라이저를 그대로 씀) 위에 base colour를 샘플링해 픽셀마다 **채색량**(0-1)을 얻고, 그 채색량이 깊이를 대신해 획·능선 규칙에 들어간다(채색 픽셀이 낮은 곳이 되도록 높이 = −채색량; `incision_sign` −1이 그린 선을 읽는다). `texture_paint` 블록: `atlas`, `colour_map`(sha256·byte_length·width·height), `chroma`, `band_um`, `sampling`. `chroma`는 RGB가 채색량이 되는 규칙이고 닫힌 집합이다 — `red_over_blue/v1`(R−B: 백자 위의 금채·철채), `blue_over_red/v1`(B−R: 청화), `darkness/v1`(1−휘도: 철화·흑상감), `saturation/v1`(max−min: 흰 바탕 위의 모든 채색 — 주홍 열매·초록 잎·주홍 띠처럼 한 그릇에 여러 색이 있어 붉음·파람 어느 한 규칙으로는 다 못 잡을 때의 완드). 밝기 모델이 아니라 지도가 말하는 수를 자른 것이며, 문턱은 읽는 쪽의 것이다: 채색 읽기에서 `detection_policy.depth_um`은 **채색량 문턱(천분율)** 이다. **선은 가운데, 면은 가장자리**: `depth_um` 이상 칠해진 픽셀의 연결 성분 가운데 폭(거리 변환 최댓값의 두 배)이 `band_um`(기본 800)을 넘는 것은 선이 아니라 면이므로 경계 안쪽 1.5 px의 띠만 남기고 속을 비워, 능선 규칙이 그 가장자리를 선으로 읽는다(구연의 금채 띠는 위아래 두 선). `texture_relief`의 `smoothing_um`은 채색 읽기에 없다. QC는 `texture_paint_covered_pixel_count`·`texture_paint_painted_pixel_count`·`texture_paint_wide_area_count`·`texture_paint_unmatched_corner_count`이고 릴리프의 높이·적분 QC는 없다. `validate_texture_lines_recipe`는 두 블록 중 하나만 허용한다. `compute_texture_lines(session, atlas, None, colour_map=…, chroma=…, band_mm=…)`. record type·payload·도판은 시문선과 같다 — 채색선도 내선이다.
- **패턴 묶기** (`pattern` = `strokes_by_direction_neighbourhood/v1`, 획 규칙·능선 규칙에서): 전개면 위에서 두 획의 가운데가 `pattern_gap_um`(기본 6,000) 안에 있고 방향(mod 180°)이 20° 안이면 이웃이고, 이웃 관계의 연결 성분 중 획 4개 이상이 패턴이다. 패턴은 벽의 위에서 아래로, 다음 왼쪽에서 오른쪽으로 번호를 받고, 어느 패턴에도 들지 않은 획은 **느슨한 획**(−1)이다. 각 패턴의 `direction_deg`(0-179)와 `line_count`를 적는다. 어골문의 좌우로 기운 띠는 띠마다 제 패턴이 된다 — 사람이 그렇게 보는 것과 같다. **패턴을 가로지르는 선은 패턴이 아니다**: 추적한 사슬이 12 mm 이상이고 양 끝 거리가 길이의 60%에 못 미치며(헤맴) 꺾이는 자리가 하나 이하이면 — 어골문의 지그재그는 꺾임이 여럿이라 여기 들지 않는다 — 복원한 조각의 접합선·균열·긁힘으로 보아 잇거나 자르지 않고 그대로 `pattern_of` −2(**seam**)로 따로 둔다. 이웃 획보다 몇 배 긴 것을 접합선으로 보는 규칙은 시험해 보고 버렸다: 빽빽한 무늬에서는 어골문의 긴 팔이 짧은 팔 곁에서 튀어 어골문 절반이 접합선이 됐다. 곧은 접합선은 획으로 남으니 검수가 마지막 말이다. 버리지 않고 남기되 도판에서는 내선이 아니라 균열(`condition_crack`)로 그리고, 검수에서 −2로 뺄 수 있다. **문양대**(`bands`): 높이가 겹치거나 3 mm 안에 있는 패턴들이 한 문양대이고, 문양대마다 `height_um_min`·`height_um_max`·`patterns`(패턴 번호, 오름차순)·`directions_deg`(그 패턴들의 방향, 오름차순·중복 없음)를 적으며 벽의 위에서 아래로 늘어선다. 모든 패턴은 정확히 한 문양대에 든다. 토기 두 점을 획이 아니라 구성 — 어떤 문양대가 어떤 차례로 — 으로 견줄 수 있게 하려는 것이다. 문양의 이름(단사선문·어골문)은 적지 않는다: 무엇이 비슷한가와 그것을 무엇이라 부르는가는 다른 일이고, 이름은 실측자가 붙인다.
- **payload** (`org.archmeshrubbing:texture-lines-v1`, schema 1.0.0; 획 규칙은 **1.1.0**): `view`와 `polylines`. 1.1.0은 여기에 선마다의 패턴 번호 `pattern_of`(느슨하면 −1, 가로지르는 선이면 −2)와 `patterns`(`direction_deg`·`line_count`), `bands`를 더하고, QC에 `pattern_count`·`loose_line_count`·`seam_line_count`·`band_count`가 붙는다. 1.0.0 payload에는 이 열쇠가 없어야 하고 descriptor의 schema와 payload의 schema가 같아야 한다. 선 하나 없는 읽기는 record가 되지 않는다. 상한은 선 20,000, 점 500,000. `geometry_ref`는 payload sha256이고 QC는 `line_count`·`point_count`·`total_length_um`·`view`에 더해 골 픽셀 수, 보이는 면 수, 높이 범위, 적분 부적합이다. 읽어 들일 때 descriptor의 byte_length·sha256·geometry_ref·recipe·QC를 대조한다(`texture_lines_payload_from_record`, `validate_known_records`).
- **도판**: `DrawingSheetOptions.texture_line_records`. record의 뷰와 frame이 같은 외곽선 도형에만 그리고 단면에는 그리지 않는다. 미러 도형에서는 입면 반쪽에 떨어진다. 선 종류는 능선과 같이 내선(`outline_hole`)이다. **패턴은 제 `<g>`다**: 1.1.0 payload의 선은 내선 layer 안에서 패턴마다 `<g id="texture-pattern:{record}:p00">`(느슨한 획은 `:loose`)로 묶이고, 가로지르는 선(−2)은 균열 layer(`condition_crack`) 안의 `:seam` 묶음이며 선 id는 `texture-line:{record}:p00:00000`이라, 편집 프로그램에서 패턴 하나를 통째로 고르거나 지울 수 있다. 1.0.0의 선은 예전 id 그대로 묶음 없이 그려진다. **검수로 지우기**: `texture_line_hidden_patterns=((record, 패턴 번호),…)`(느슨한 획은 −1)에 든 패턴은 그리지 않고, sidecar `texture_lines.hidden_patterns`와 `drawn[].hidden_patterns`·`drawn[].drawn_polyline_count`가 무엇을 뺐는지 적는다 — 프로그램이 읽고 실측자가 정하되, 정한 것이 기록에 남는다. sidecar `texture_lines.records`(record ID·recipe hash·payload sha256·선 수·1.1.0이면 `pattern_count`·뷰)와 `texture_lines.drawn`. 옵션이 비면 sidecar에 키가 없고 바이트가 그대로다. 시트의 `line_smoothing_mm`은 이 선에 걸리지 않는다 — recipe의 `line_smoothing_um`으로 이미 평활된 선이다.
- 완료 게이트(3/6/6)와 `.amr-survey`에는 들어가지 않는다.

## 단면 꺾임 record `measurement.profile_break.v1`

회전체의 바깥은 축 둘레로 돌린 단면 r(z)이고, 그 단면이 꺾이는 자리 — 굽이 몸체에서 나뉘는 뿌리, 굽이 튀어나온 모서리, 어깨의 능 — 는 그릇을 한 바퀴 돈다. 실측 도면은 그것을 입면에 **수평 내선 한 줄**로 긋는다: 실루엣에서 축까지, 옆에서 본 원. 홈이 아니고(파인 것이 없다) 외곽선도 아니며(거기서 끝나는 것이 없다) 내선이고, 뷰가 아니라 축의 사실이다. 안쪽도 단면이 있다 — 내바닥이 벽과 만나는 뿌리, 뚜껑이 앉는 턱, 어깨 안쪽의 접힘 — 그리고 그것은 단면 반쪽에서 잘린 벽 너머로 보이는 내면에 **축에서 내벽까지** 같은 수평 내선으로 긋는다. 읽기 하나가 어느 쪽 면을 읽었는지 밝힌다.

- **recipe**: `archmeshrubbing.axial_profile_break` **1.0.0**, 좌표계 `canonical_mm_axis_profile/v1`. `profile_policy`(`height_bin_um` 기본 250, `surface` — `outward_facing_every_shell/v1`(기본): 법선이 축에서 멀어지는 모든 면, 껍질을 가리지 않는다: 굽은 흔히 제 껍질이고 굽이 몸체와 만나는 모서리가 바로 꺾임이므로 단면이 껍질의 가장자리에서 멈추면 안 된다; 또는 `inward_facing_every_shell/v1`: 법선이 축을 향하는 모든 면 — 내벽, 굽 안쪽 면. 어느 쪽이든 법선의 반지름 방향 성분이 0.25 미만인 면(구연 윗면·내바닥·밑면)은 어느 단면에도 들지 않는다, `radius_statistic` 회전 중앙값, `minimum_bin_sample_count`), `detection_policy`(`angle_min_deg` 기본 25, `span_um` 기본 1,500 — 한 표본의 아래·위로 이 폭만큼 떨어진 표본까지의 현(弦) 둘이 그 자리의 방향이고, 두 현 사이 각이 문턱 이상이면 꺾임; `direction` = `chord_over_span_either_side/v1`, `keep` = `sharpest_within_span/v1` — 한 굽이의 표본 가운데 폭 안에서 가장 크게 꺾이는 하나만 남긴다). 양 끝(구연·바닥)은 현이 닿지 않으므로 꺾임이 아니다. `validate_profile_break_recipe`는 수들로 다시 만들어 같은 바이트를 요구한다.
- **payload** (`org.archmeshrubbing:profile-break-v1`, schema 1.0.0): `breaks`(높이 오름차순; 각각 `height_um`, `radius_um`, `turn_millidegrees` — 읽은 면에서 튀어나온 **볼록** 모서리가 양수, 면이 접혀 드는 **오목** 뿌리가 음수. 바깥 면에서는 (r, z) 반평면에서 위로 가며 왼쪽으로 도는 각이 양수(벽이 축에서 멀어지는 쪽으로 꺾임)이고, 안쪽 면은 재료가 반대편에 있으므로 같은 회전이 뿌리라 부호를 뒤집는다 —, `revolution_spread_um` 그 높이 반지름의 사분위 폭), `profile_bin_count`, `profile_minimum_height_um`, `profile_maximum_height_um`. 꺾임 하나 없는 읽기는 record가 되지 않는다(`angle_min_deg`를 내리라고 말한다). QC는 `break_count`, `convex_break_count`, `concave_break_count`, `break_heights_um`, `maximum_turn_millidegrees`, `maximum_revolution_spread_um`. `geometry_ref`는 payload sha256. 축 Align이 있어야 하고, Align이 바뀌면 stale이다(홈 record와 같다).
- **도판**: `DrawingSheetOptions.break_records`. 꺾임마다 그 높이·반지름의 원을 도형 frame에 사영한 현(`axis_profile_chord`)을 내선(`outline_hole`)으로 `profile-break:{record}:000` id로 긋는다. 축이 놓인 평면의 도형에만 그리고 평면도에는 아무것도 그리지 않는다. 어느 면을 읽었는가가 어디에 그릴지를 정한다: 바깥 면의 꺾임은 입면(outline 도형, 미러 도형의 입면 반쪽 — 축에서 잘린다; `break_reach`를 `section`으로 두면 외형선의 가장자리처럼 가운데선을 지나 단면선 앞까지 이어진다, 기본 `axis`)에, 안쪽 면의 꺾임은 단면(미러 도형의 단면 반쪽 — 축에서 내벽까지, 가운데선이 꺾인 상자 안은 잘려 나간다 — 또는 홀로 그린 cutline 도형 — 내벽에서 내벽까지)에 긋는다. 안쪽 꺾임선은 내벽의 단면선에서 종이 1 mm 앞에서 멈춘다. 꺾임의 각이 `break_solid_min_deg`(기본 30) 이상이면 실선, 미만이면 한 번, 그 절반 미만이면 두 번 끊어 긋는다(틈 종이 1 mm; id `profile-break:{record}:NNN:l00`·`:r00`처럼 축 양쪽 반쪽과 조각 번호). `break_styles`의 `(record ID, 꺾임 번호, 스타일)`이 꺾임 하나의 규칙을 덮는다: `solid`·`broken_once`·`broken_twice`·`omit`; 없는 record·번호는 거부. sidecar `profile_breaks.records[].lines[]`(`index`, `height_um`, `turn_millidegrees`, `style`, `source` = `rule`·`choice`), `drawn[]`의 `chosen_count`·`omitted_count`, 전부 뺀 record는 `not_drawn`에 `every_corner_omitted`. 입면만 있는 도형은 안쪽 꺾임을 보일 수 없고, 단면만 있는 도형은 바깥 꺾임이 필요 없다(잘린 면이 이미 그린다); 둘 다 sidecar `profile_breaks.not_drawn`에 이유(`interior_needs_section_half`, `exterior_needs_elevation`)와 함께 남는다. sidecar `profile_breaks.records`(record ID·recipe hash·payload sha256·`break_count`·`break_heights_um`·`surface`)와 `profile_breaks.drawn`(`half` = `elevation`·`section`·`figure`, `surface`). 옵션이 비면 sidecar에 키가 없다.
- 완료 게이트(3/6/6)와 `.amr-survey`에는 들어가지 않는다.

## 채색 문양 이미지 record `measurement.paint_cutout.v1`

그린 것을 모두 선으로 옮기지는 않는다. 붓으로 쓴 묵서, 촘촘한 문양은 있는 그대로 보이는 편이 낫다 — 이미지 편집기의 완드로 그 자리만 따내듯 base colour에서 **채색된 픽셀만** 오려 제자리에 붙인다. 톤은 읽기가 정한다: 도면의 톤에 맞춘 단색 먹(채색량이 농담)이거나, 완드로 선택한 그대로 **채색의 제 색**이거나. 백자 굽 안의 묵서 '天'을 도면 아래에 따 붙인 보고서 도판이 그 관례다.

- **recipe**: `archmeshrubbing.view_paint_cutout` **1.0.0**, 좌표계 `view_plane_mm/v1`. `view`(여섯 뷰 중 하나 — 이화문은 `front`, 굽 안 묵서는 `bottom`), `raster_policy`(`pixels_per_mm` 기본 10, `facing_cos_millionths` 기본 350,000, `painter` = `far_to_near_facing_faces/v1` — 뷰를 향한 면을 먼 것부터 칠해 가까운 벽이 이긴다), `texture_paint`(내선 record의 채색 블록과 같되 `band_um`은 0 — 면을 가장자리로 줄이지 않고 채색을 통째로 둔다), `ink_policy`(`threshold_thousandths` 기본 60 — 여기서 먹이 시작, `full_thousandths` 기본 250 — 여기서 먹이 꽉 참, 그 사이는 선형 `ramp`; `margin_um` 기본 1,000 — 채색 범위 둘레의 여백; `tone` — `ink/v1`(기본, 단색 먹) 또는 `colour_as_painted/v1`(base colour의 제 색 그대로, 덮임은 알파)), `window`(null이거나 `left_um`·`bottom_um`·`right_um`·`top_um` — 뷰 mm의 상자 안 채색만 따낸다: 원하는 문양 하나만, 뷰에 보이는 채색 전부가 아니라), `source_vertex_count`·`source_face_count`. `validate_paint_cutout_recipe`는 수들로 다시 만들어 같은 바이트를 요구한다.
- **raster**(`PaintCutoutRaster`, 문서 밖에서 record 곁을 따라다닌다 — 띠 탁본과 같다): 먹이면 GA8(`gray8_alpha8_ink_coverage/v1`, 회색 0에 알파가 채색량의 덮임), 제 색이면 RGBA8(`rgb8_alpha8_painted_colour/v1`, 지도의 RGB 그대로에 같은 알파; 덮임 0인 픽셀은 색도 0), 행 0이 뷰의 위. 채널 수가 `pixel_format`을 정하고 receipt의 `pixel_format`은 recipe의 `tone`과 맞아야 한다. `left_um`·`bottom_um`이 뷰 mm에서 종이의 왼쪽·아래 모서리. `raster_sha256`은 의미 헤더와 픽셀의 해시.
...
- **도판**: `DrawingSheetOptions.paint_cutouts=((cutout record, 도형 record, 자리),…)`, 자리는 `in_place`(뷰가 같은 도형의 제자리 — 미러 도형에서는 입면 반쪽 안이거나 꺾음 상자 안이어야 하고, 아니면 거부: 단면 반쪽 위에 채색이 놓이면 안 된다) 또는 `below`(도형 아래 3 mm 띄워 가운데 — 저부 묵서처럼 다른 뷰의 세부). `paint_cutout_ink_percent`(기본 70, 20-100)가 먹 cutout의 `<image opacity>`로 진하기를 정한다 — 선 위에 튀지 않게; 제 색 cutout은 완드로 뜬 그대로 100%로 붙고(RGBA PNG) 이 값을 받지 않는다. 이미지는 선 layer보다 먼저 놓여 선이 그 위에 그려지며 id는 `paint-cutout-{도형}-{n}`, `data-placement`. sidecar `paint_cutouts.records`(record ID·recipe hash·raster sha256·뷰·크기)와 `paint_cutouts.drawn`(도형·자리·먹·`tone`·`rectangle_um`). 옵션이 비면 sidecar에 키가 없다.
- 완료 게이트(3/6/6)와 `.amr-survey`에는 들어가지 않는다.

## 요철 음영 record `measurement.relief_shade.v1`

요철 문양은 선이 아니다. 청자 접시의 연판문처럼 벽에서 도드라진 문양은 점을 무수히 찍어 양감을 살린다 — 빛에서 돌아서는 자리는 촘촘하게, 빛을 받는 자리는 비워서. 이 record는 그 점이 따르는 **음영**만 읽는다; 점을 어떻게 찍을지는 도판의 일이고 도판의 축척에서 정해진다.

- **recipe**: `archmeshrubbing.view_relief_shade` **1.1.0** (1.0.0은 뷰의 스침 컷과 원근 보정을 행의 중앙값 반지름에 대어 잰 것이라, 중앙값보다 넓게 선 뒤틀린 그릇의 한쪽 바깥 띠를 통째로 잘랐다; 1.1.0은 `base_policy.silhouette` = `each_side_of_row/v1` — 픽셀이 놓인 쪽 실루엣까지의 거리로 잰다, `grazing_fraction_thousandths` 기본 980; 뷰의 양각도 전개처럼 **반지름**으로 잰다 — 깊이에서 기준 깊이를 빼면 실루엣 근처에서 기준 깊이가 급히 떨어져 거짓 양각이 되지만, 픽셀이 뜻하는 반지름에서 기준 반지름을 빼면 어디서나 같다). `domain`이 셋이다. `view/v1`: 좌표계 `view_plane_mm/v1`, `view`는 옆 뷰 넷(`front`·`back`·`left`·`right`) 중 하나 — 기준면이 canonical 축의 회전체이므로 평면·저면 뷰는 받지 않는다. `axis_plan/v1`: 좌표계 `view_plane_mm/v1`, `view`는 `top`·`bottom` 중 하나 — **뚜껑 상면처럼 축을 내려다보고 읽는 면**이다. 평면 뷰의 그리는 평면은 유물 자신의 x-y이므로 픽셀이 가운데에서 떨어진 거리가 곧 유물의 반지름이고, 회전체는 고리마다 높이가 하나다: 그 고리의 중앙값 높이가 기준면이고(`plan_policy.model` = `revolution_ring_median/v1`, 고리 폭은 픽셀 하나, `base_policy.row_smoothing_um`로 반지름 방향으로 고른다), 그보다 뷰어에 가까운 픽셀이 도드라진 것이다. `plan_policy.ring_inner_um`·`ring_outer_um`이 반지름 창 — 뚜껑의 꼭지를 빼고 가장자리가 꺾여 내려가기 전에서 멈추는 가위다(0이면 제한 없음). QC는 `plan_ring_inner_um`·`plan_ring_outer_um`과 반지름 17칸의 `plan_ring_radius_um`·`plan_ring_base_um`, 그리고 `base_height_min/max_um`(옆 뷰의 `base_radius_min/max_um` 대신). `axis_development/v1`: 좌표계 `axis_development_mm/v1`, `view`는 null이고 `development_policy`가 있다 — 벽을 축 둘레로 펴서 한 바퀴의 문양을 한 띠에 놓는 전개도: `seam_millideg`(기본 90,000 — 띠를 자르는 자오선, 정면 뷰의 뒤쪽이라 입면이 보이는 앞면이 띠 가운데 통째로 온다), `profile_bin_um`(기본 250 — 바깥면 중앙값 단면을 읽는 높이 칸), `outward_cos_thousandths`(기본 250 — 법선이 축에서 이만큼 멀어지는 면이 바깥 벽), `model` = `cylindrical_at_window_middle/v1`(벌어진 벽의 참 전개는 부채꼴이지만 실측자의 띠는 곧다: 창 높이 한가운데의 단면 반지름을 기준 반지름으로 삼은 원통으로 펴 세로선은 세로로 남고 너비는 벽의 반지름 비로 늘고 준다), `direction` = `counterclockwise_from_seam/v1`. u는 기준 반지름 × 이음매에서의 각, v는 단면을 따라 올라간 자오선 호 길이, 깊이는 정점의 제 반지름이라 단면보다 도드라진 것이 띠에서도 도드라지고, 언더컷 면은 같은 자리의 벽 면에 픽셀을 내준다. 기준면은 행마다 둘레의 중앙값 반지름. `window`의 아래·위는 이 domain에서는 뷰 mm가 아니라 유물의 **높이**이고(행은 호 길이라 안에서 바꾼다) 좌·우는 띠 위의 u mm다. `raster_policy`(`pixels_per_mm` 기본 10, `facing_cos_millionths` 기본 50,000, `painter` = `front_most_depth/v1` — 뷰를 향한 면을 뷰 격자에 사영해 픽셀마다 가장 앞의 깊이, `layer_separation_um` 50, `margin_um` 기본 1,000), `base_policy`(`model` = `revolution_row_median/v1` — 행마다 가까운 벽의 깊이가 뜻하는 반지름 √(깊이² + x²)의 중앙값을 실루엣 반폭의 `inner_fraction_thousandths`(기본 600) 안에서 읽고 `row_smoothing_um`(기본 300)으로 위아래로 고른다; 실루엣의 `grazing_fraction_thousandths`(기본 920) 밖은 벽이 모로 보여 버리고, 기준면에서 `outlier_um`(기본 10,000) 넘게 떨어진 픽셀 — 수평이 아닌 구연 너머로 보이는 먼 내벽 — 은 양각이 아니다), `relief_policy`(`slow_um` 기본 40,000 — 어느 문양보다 느리게 그릇을 도는 것(타원 구연, 스캔의 기울기: 둘레에 한 번)을 빼는 뷰 가로의 **이동 중앙값** 창 — 블러는 문양마다 거짓 골의 후광을 남기지만 문양 몇 개 폭의 중앙값은 그 자리에서 벽이 서 있는 높이다; 덮이지 않은 픽셀은 행의 중앙값으로 채워 표를 던지지 않고, 위아래로는 걸지 않는다 — 단면은 기준면이 이미 뺐고 구연 밑이나 굽 뿌리의 턱을 가로질러 걸면 거짓 양각의 띠가 생긴다, `grain_um` 기본 400 — 스캔의 결을 빼는 좁은 블러; 덮이지 않은 픽셀이 값을 끌어내리지 않게 마스크로 나눈 블러, `slow_blend_um` 기본 1,000 — 행마다 따로 잰 느린 높이를 위아래로 섞는 폭; 없으면 행의 중앙값이 가로 줄무늬로 찍힌다, 문양(15 mm)보다 훨씬 좁다), `shade_policy`(`model`이 둘 — `lambert_height_field/v1`은 법선 (−∂h/∂x, −∂h/∂y, 1)의 램버트 조명이고, **`contact_envelope_ink/v1`은 탁본의 농담**이다: 종이를 눌러 얹으면 제 폭보다 좁은 것은 건너뛰므로 종이가 놓이는 면은 요철 자신의 위 포락선(`paper_um` 기본 700 — 종이가 건너뛰는 폭)이고, 먹은 그 아래로 얼마나 내려앉았는지에 따라 옅어져 `black_point_um`(기본 120)에서 사라지며, 종이가 닿은 자리의 짙기가 `contact_ink_thousandths`(기본 700)다. 빛은 비탈이 어느 쪽을 보는지를 말하므로 찍어 낸 문양은 한쪽 비탈만 보이지만, 접촉은 종이가 무엇에 닿았는지를 말하므로 **문양이 통째로** 읽힌다 — 바탕이 짙고 눌린 자리가 옅다. 인화문·시문처럼 찍거나 그은 문양은 이쪽이다. 수치는 탁본 record의 접촉 모델에서 그대로 가져왔다, `light_thousandths` 기본 [−1000, 1000, 1200] 왼쪽 위에서 앞으로, `foreshortening` = `arc_cos_corrected/v1` — 뷰 가로의 기울기를 축 둘레 각의 코사인으로 눌러 실루엣 쪽의 과장을 없앤다, `gain_thousandths` 기본 2,000 — 평평한 벽의 밝기에서 모자란 몫 × 이득이 어둡기, `cavity_um`(기본 0 = 끔)과 `cavity_gain_thousandths`(기본 1,000) — 문양 사이의 골은 문양의 그늘에 있으므로 기준면보다 낮은 만큼(이 깊이에서 꽉 참) 더 어둡게, `floor_thousandths` 기본 60 — 이보다 옅은 음영은 메쉬 면이나 결이지 음영이 아니다, `edge_erosion_pixels` 기본 2 — 다만 결 블러 폭의 두 배보다 가장자리에 가까운 픽셀은 언제나 믿지 않는다), `window`(null이거나 뷰 mm 상자 — 문양이 있는 띠만; 구연 밑이나 굽 뿌리는 선이 이미 그린다), `source_vertex_count`·`source_face_count`. `validate_relief_shade_recipe`는 수들로 다시 만들어 같은 바이트를 요구한다.
- **raster**(`ReliefShadeRaster`, 문서 밖에서 record 곁을 따라다닌다): GA8(`gray8_alpha8_shade/v1`, 회색 0에 알파가 어둡기), 행 0이 뷰의 위, `left_um`·`bottom_um`이 뷰 mm에서 종이의 왼쪽·아래. `raster_sha256`은 의미 헤더와 픽셀의 해시.
- **record**: extension `org.archmeshrubbing:relief-shade-v1`에 receipt만. `geometry_ref`는 raster sha256. 도판에 `rasters={record: ReliefShadeRaster}`로 주면 `require_relief_shade_raster`가 대조한다. QC는 `shaded_pixel_count`, `darkness_mean_thousandths`, `relief_p05/p50/p95_um`·`relief_max/min_um`(필터 뒤), `base_radius_min/max_um`, `covered_pixel_count`, `trusted_pixel_count`, `visible_face_count`. 음영이 하나도 없으면(벽이 빛에 평평하면) record가 되지 않는다. 축 Align이 바뀌면 stale이다.
- **도판**: 뷰 domain은 `DrawingSheetOptions.relief_stipples=((shade record, 도형 record),…)` — 같은 뷰의 도형에 제자리로. 전개 domain은 도판의 record 목록에 shade record ID를 넣어 **제 도형**으로 그린다(띠 전체를 점으로, 아래에 캡션 `요철 음영 전개 · 이음매 90° · 이득 · 골 · 높이`; 뷰 도형에 붙이면 거부, 뷰 domain을 제 도형으로 그리면 거부). QC `development_reference_radius_um`·`development_circumference_um`·`development_arc_um`·`seam_millideg`, 그리고 raster의 아래 가장자리에서 위 가장자리까지 고르게 잡은 17칸의 `development_height_profile_um`·`development_radius_profile_um` — 그 행이 유물의 어느 높이에서 왔고 거기서 벽이 축에서 얼마나 멀었는지. **띠를 입면에 붙이기**: `DrawingSheetOptions.relief_developments_on_axis=((음영 record, 입면 record),…)`가 뷰를 마주 보는 자오선을 가운데 선에 놓고 그 반쪽 쪽으로 호 길이대로 편 다음, 그 띠의 벽 반지름 중 가장 큰 값 한 곳에서 곧게 자른다 — 종이의 가장자리는 곧으므로 붙는 것은 직사각형이고, 낮고 넓은 띠는 가로로 긴 직사각형이 된다. 행의 세로 자리는 위 두 profile에서 나오므로 그 값이 없는 옛 record는 거부한다. 붙인 띠는 제 도형으로 또 실을 수 없고, 뷰 domain 음영을 이 옵션에 주면 거부한다(그건 `relief_stipples`의 몫이다). sidecar에는 `relief_stipples.drawn`에 `half` = `elevation`로 들어가고, 자른 자리는 `cut_radius_um`(붙인 띠에만 있는 키), 잘려 나간 점은 `dropped_section_side_count`가 센다. 점은 종이 격자 `stipple_pitch_mm`(기본 0.15, 잠정) 칸마다 한 번의 기회로 놓인다: 칸 번호의 해시(`splitmix64_cell/v1`)에서 나온 균등수 셋으로 칸 안의 자리 둘과 기회 하나를 얻고, 그 자리의 어둡기보다 기회가 작으면 점이 선다 — 같은 음영은 같은 축척에서 언제나 같은 점이다. 미러 도형에서는 접힌 선의 입면 쪽 점만 남고 단면 쪽은 세어서 뺀다. 점은 `<g id="relief-stipple-{도형}-{n}">` 안의 `<circle>`(지름 `stipple_dot_mm`, 기본 0.12, 잠정)로 선 layer보다 먼저 놓인다. sidecar `relief_stipples`(`pitch_paper_mm`, `dot_paper_mm`, `hash`, `drawn[]`의 `dot_count`·`dropped_section_side_count`·`half`, `records[]`). 옵션이 비면 sidecar에 키가 없고 SVG 바이트가 같다.
- 완료 게이트(3/6/6)와 `.amr-survey`에는 들어가지 않는다.
...
- `python main.py --plate PROJECT.amr SPEC.json OUT.svg`가 프로젝트와 명세만으로 도판을 다시 만든다(`src/application/plate_from_spec.py`). 선 도면은 문서만 있으면 되고, 탁본은 record가 픽셀을 저장하지 않으므로 GUI의 내보내기와 똑같이 recipe로 다시 계산해 영수증과 맞지 않으면 거부한다 — 그래서 탁본이 있는 도판은 원본이 동봉된 프로젝트가 필요하다. 채색 따 붙이기와 요철 점묘의 픽셀은 프로젝트에 없는 색 지도와 음영에서 오므로 명령줄은 거부하고, 그것을 계산한 자리에서 만들라고 말한다. `write_plate_spec`은 사람이 읽게 들여쓴 파일로 쓰되 정규 순서로 쓰고, 읽을 수 없는 것은 쓰지 않는다.

## 판독 일곱 가지와 실측자 이름

도판은 오래전부터 꺾임·홈·능선·뒷면 실루엣·요철 음영·문양 내선·채색 따내기를 그렸지만, **응용 계층에는 그것을 만드는 길이 없었다** — record 종류도 있고 composer도 그렸는데, 하나 만들려면 파이썬을 써야 했다. `src/application/artifact_readings.py`가 그 구멍을 메운다.
...
- 세션은 새것을 돌려준다(`ReadingOutcome.session`). 원래 문서는 건드리지 않는다. 요철 음영의 픽셀은 record가 영수증만 저장하므로 `ReadingOutcome.raster`로 따로 온다.
