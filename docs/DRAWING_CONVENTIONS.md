...
**방향은 관찰한 사실이다.** 목리조정흔·물손질흔은 도구가 움직인 방향으로 긋는다. 방향은 record에 담는다(`direction_deg`, payload 1.1.0; 모르면 null). 도판의 `technique_angles_deg`가 있으면 그것이 이기고, 없으면 record의 방향, 그것도 없으면 종류별 기본값(목리 90°, 물손질 0°)이다. 타원과 이음선은 방향이 없다. 앱에서는 도판 항목의 "기법 획 방향 지정"을 켜고 각도를 넣으면 그 도판의 기법 record 전부에 적용된다. 기록 시점에 방향을 넣는 칸은 아직 패널에 없다.
...
- **패널에서 방향 넣기.** record는 방향을 담을 수 있지만(`direction_deg`), 기록 버튼 옆에 각도 칸이 아직 없다.
