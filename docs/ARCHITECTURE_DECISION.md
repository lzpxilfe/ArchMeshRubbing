...
제품·문서·배포·차단 CI 대상은 Windows x64 하나다. 지원 운영체제는 Windows 10 version 1809 이상 x64와 Windows 11 x64이며, 최소 버전은 고정한 Qt 6.11의 [공식 지원 플랫폼](https://doc.qt.io/qt-6.11/supported-platforms.html)을 따른다. 배포 경로는 검증형 Windows onedir portable ZIP 하나로 제한한다. Windows ARM64·32-bit·Server 최종 사용자 환경, 비 Windows 운영체제와 호환 계층, installer·MSIX·Store 패키지는 비목표다. 코드에 남은 비 Windows 분기와 backend는 내부 회귀 시험·참조 구현일 뿐 이 결정의 지원 계약이 아니다.
...
| `.amr`, `.amr-vector`, `.amr-rubbing`, `.amr-survey`, `.amr-unwrap` 검증·원자 저장 | screenshot/OpenCV/convex-hull을 실측 산출물로 쓰는 우회 경로 |
...
1. `Application state`: 현재 document/session과 project path, canonical document SHA-256/정규화 path의 저장 checkpoint, 단일 pending Open, 활성 revision, 작업 가능 여부만 소유한다.
2. `Commands`: Open, Align commit, Cutline/Outline/Rubbing, 검증 제원, 표면 anchor 거리/지름 compute와 export를 명시적 입력·결과로 실행한다.
...
현재 `src/application/artifact_workbench.py`는 Qt·OpenGL 없이 session/project authority, ticketed Open, `state_version`/`authority_epoch` compare-and-swap, 명시적 Align readiness와 two-phase projection publication을 소유한다. Open/new import/project reopen과 Align commit/parent activation은 이 경계를 사용한다. Open이 만드는 `initial_identity`는 materialization baseline일 뿐 사용자 확정이 아니며, 변화량이 0인 첫 Align도 immutable child revision으로 남겨야 측정 단계가 열린다. 전체 메쉬 제원과 point 측정도 이 경계를 우회하지 않는다. `ArtifactMeasurementController`는 fresh canonical-mm projection으로 `measurement.geometry_metrics.v1`을 계산하며 topology가 체적 조건을 충족하지 않으면 legacy convex-hull 근사로 fallback하지 않는다. 거리·지름은 exact frame에서 source triangle row와 10억 분율 barycentric anchor를 고정하고 각각 `measurement.surface_distance.v1`의 3D Euclidean chord와 `measurement.circle_diameter.v1`의 PCA best-fit plane 위 정규화 대수 Kasa 원을 게시한다.
...
현장 파일럿은 새 측정 authority를 만들지 않는다. `src/core/field_pilot.py`는 이미 검증된 self-contained `.amr`, exact-project `.amr-survey`, 동일 runtime lock인 Windows native OpenGL v2 report와 고고학자의 닫힌 review를 읽어 단일 증거 report로 결합한다. 현재 build commit이 알려져 있으면 OpenGL report도 `unknown`이 아닌 exact commit이어야 하고, frozen manifest가 있으면 `source_tree`도 exact match해야 한다. OpenGL v2는 성공 root, 현재 87개 필수 actual-frame check ID, perspective/top-orthographic receipt와 Windows runtime self-claim을 닫으며, 이 runtime이 report machine과 exact match하고 지원 client 계약을 만족하며 24시간 안에 생성돼야 한다. 별도 `src/windows_runtime.py` 계약은 저장된 Windows client build/product type, native/process architecture, Wine export 탐지와 Python 구현·버전으로 Windows 10 build 17763+/Windows 11 Workstation·native AMD64·64-bit AMD64 process·CPython 3.12·비호환 계층을 결정적으로 재검증한다. report는 canonical self-hash와 `authentication=none`, `single_artifact_single_machine`, `single_pilot_only_not_release_approval`을 강제한다. hostname·사용자명·절대 path는 자동 수집하지 않으며, runtime match는 물리 장비 인증이 아닌 실수 재사용 방지 self-claim이다. human/driver/runtime 근거가 없으면 package self-test에서도 완료로 승격하지 않는다. 이 contract가 실제 대표 유물 pilot을 대신하지는 않는다.

Native project Save는 GUI가 immutable `ArtifactSession`, project path와 Workbench state/authority version을 캡처한 뒤 기존 fail-closed project writer 전체를 worker에서 실행한다. 따라서 live/source geometry exact comparison, Git metadata subprocess, source closure 재해시, ZIP64 streaming, staging file `fsync`, staged package production-reader 재개방과 source/Align materialization이 GUI event loop를 점유하지 않는다. Windows writer는 source archive descriptor를 닫은 다음 같은 parent staging과 목적지의 긴 Unicode/UNC 경로를 Win32 extended path로 넘겨 `MoveFileExW(MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)` 하나로 commit한다. Win32 호출 실패는 pre-commit 실패이며 기존 목적지를 보존하고 `os.replace`로 downgrade하지 않는다. 성공한 commit 뒤 authority가 바뀌었으면 이미 기록된 파일을 과거 snapshot으로 명시하고 현재 Save As 경로·migration flag를 채택하지 않는다. 완료 경로는 Workbench의 `adopt_saved_project_path` compare-and-swap으로 canonical document SHA-256, 정규화 project path, `confirmed | uncertain` 내구성을 갖는 exact snapshot checkpoint를 갱신하며 path/checkpoint state version을 전진시키되 document/render authority epoch는 유지한다. 같은 경로 Save와 Save As는 동일한 CAS 계약을 사용한다. 정상 UI에서는 application-modal 진행 대화상자가 다른 명령을 막으며, programmatic authority 경합도 이 완료 fence가 숨기지 않는다. 내부 비 Windows writer는 `os.replace` 뒤 directory fsync 결과를 회귀 시험할 때만 유지하며 제품 지원·배포 완료 근거로 사용하지 않는다.

저장 여부는 별도 GUI boolean이 아니라 현재 Workbench document SHA-256/project path와 checkpoint의 exact match에서 파생한다. 새 source import는 checkpoint가 없어 dirty이고, production reader와 embedded-source materialization을 모두 통과한 project reopen은 confirmed clean이며, immutable Align/record 추가는 document SHA-256을 바꿔 dirty가 된다. 이 파생 상태를 Windows native 창 제목 `*`와 status label이 같이 구독한다. Close, 새 source Open, Project Open, drag-and-drop은 dirty 문서에서 `Save / Discard / Cancel` 결정을 공유한다. `Save`를 고른 비동기 후속 명령은 exact captured snapshot의 confirmed checkpoint가 현재 authority와 일치할 때만 호출한다. 저장 대화상자 취소, writer/CAS 실패, stale 완료, durability-uncertain은 이 호출 권한을 만들지 않으므로 현재 문서와 창을 보존한다.

비정상 종료 save-temp 복구는 project writer의 은닉 파일을 자동 소유물로 간주하지 않는다. 사용자가 고른 단일 폴더에서 exact writer basename을 가진 regular file만 bounded discovery하고, 발견한 device/inode/size/mtime capability가 유지된 descriptor를 새 목적지 parent staging으로 복사한다. staging이 production embedded-session loader에서 source/geometry/Align까지 완전히 재현된 뒤에만 게시한다. Windows는 extended Unicode/UNC 경로의 `MoveFileExW(MOVEFILE_WRITE_THROUGH)`를 replace flag 없이 사용해 create-new와 write-through를 함께 강제하며, 목적지 경합·Win32 실패에서 `os.rename`으로 downgrade하지 않는다. 내부 비 Windows 경로의 atomic no-replace rename과 directory fsync는 회귀 시험용으로만 유지한다. 후보·기존 intended destination은 어떤 결과에서도 변경하지 않고, 새 파일을 live Workbench 권위로 채택하는 동작도 별도 Project Open 확인으로 분리한다.

측정 worker 취소는 강제 thread termination이 아니라 공통 Event와 core 고유 취소 예외를 사용한다. Event는 게시 권위를 즉시 회수하고 extractor의 명시적 Python loop/chunk 경계에서 확인된다. 기와 경로는 section circle/seam fitting, row-shift section/grid/refinement 및 export recipe 재계산에 같은 Event를 전달한다. 취소되지 않은 실행의 recipe·payload·raster·QC에는 probe가 관여하지 않는다. 현재 실행 중인 단일 NumPy/GEOS/선형대수 호출, worker preflight 내부의 단일 materialization·resource-estimate 호출, Align candidate의 source hash/materialization과 project writer 내부의 단일 file/ZIP/fsync 단계는 호출이 반환할 때까지 선점할 수 없는 구간으로 남지만 GUI thread에서는 실행하지 않는다. 창 종료는 dirty native 문서의 `Save / Discard / Cancel` 결정을 먼저 적용한다. `Save`는 현재 닫기 event를 거부하고 exact durable checkpoint 확정 후 새 닫기를 요청하며, `Cancel`과 저장 미확정은 task 취소로 넘어가지 않는다. 실제 teardown이 시작된 뒤에는 현재 authoritative 측정·vector/rubbing/survey/tile-unwrap export `TaskThread`의 one-shot cancel hook으로 record/publication 권위를 먼저 회수하고 최대 30초 동안 cooperative join을 기다린다. 취소 probe가 없는 Align 준비와 project save도 같은 bounded join에서 실제 종료를 증명해야 한다. 별도 slot인 native 원본 loader와 Project inspector는 request ID와 pending Open ticket을 먼저 회수하고 signal을 끊은 뒤 같은 시간 한도의 join을 거친다. 어느 worker든 join을 증명하지 못하면 창 teardown을 거부하고 QThread 소유권을 유지하며, 늦은 결과는 이미 회수한 권위 때문에 게시되지 않는다. native Align·측정·Save·Open의 heavy GUI-thread 작업 제거와 종료 수명주기는 완료했으며 legacy profile/slice worker의 통합 수명주기는 후속 셸 이행으로 관리한다.
